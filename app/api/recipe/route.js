...
2) 재료 조합상 실제로 함께 먹을 수 있는 요리만 제안한다. 선택 재료가 5개 이상이면 한 레시피에 주재료 4개 이하만 사용하고, 서로 어울리는 일부 재료를 우선한다. 어울리지 않는 조합이면 가장 적은 재료만 골라 무리하지 않는 계란전, 볶음, 조림 중 하나로 만든다.
...
function isValidRecipe(recipe, allowed, maxUsed) {
  return recipe && typeof recipe.recipeName === 'string' && typeof recipe.cookingTime === 'string' && Array.isArray(recipe.ingredientsUsed) && recipe.ingredientsUsed.length > 0 && recipe.ingredientsUsed.length <= maxUsed && recipe.ingredientsUsed.every(item => allowed.has(item)) && Array.isArray(recipe.steps) && recipe.steps.length >= 3 && recipe.steps.length <= 5 && recipe.steps.every(step => typeof step === 'string' && step.length > 8) && recipe.nutrition && typeof recipe.nutrition.calories === 'string';
}
...
export async function POST(request) {
  ...
  const maxUsed = ingredients.length >= 5 ? 4 : ingredients.length;
  ...
  try { const data = await response.json(); const recipe = JSON.parse(data.choices[0].message.content); return NextResponse.json(isValidRecipe(recipe, allowed, maxUsed) ? recipe : fallback); } catch { return NextResponse.json(fallback); }
}
